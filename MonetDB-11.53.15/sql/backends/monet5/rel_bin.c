...
static stmt *rel2bin_pcatrain(backend *be, sql_rel *rel, list *refs);
...
subrel_bin(backend *be, sql_rel *rel, list *refs)
{
	...
	case op_merge:
		...
		if (sql->type == Q_TABLE)
			sql->type = Q_UPDATE;
		break;
	//compression simialarity join, pca
	case op_pcatrain:
    s = rel2bin_pcatrain(be, rel, refs); 
    sql->type = Q_UPDATE;                
    break;
	case op_ddl:
...
// //PCA 训练语句的 MAL 转换, 生成 MAL: model := batcalc.pcatrain(tbl, 16);
// static stmt *
// rel2bin_pcatrain(backend *be, sql_rel *rel, list *refs)
// {   
//     mvc *sql = be->mvc;
    
//     fprintf(stderr, "\n========== REL2BIN PCATRAIN DEBUG START ==========\n");
    
//     /* 1. 解析关系树中的信息 */
//     dlist *info = rel->l;
//     if (!info || dlist_length(info) != 3) {
//         fprintf(stderr, "[REL2BIN] ERROR: Invalid info list, length=%d\n", 
//                 info ? dlist_length(info) : -1);
//         return sql_error(sql, 02, SQLSTATE(42000) "Invalid PCA training expression");
//     }
    
//     dnode *n = info->h;
//     char *tbl_name = n->data.sval;
//     fprintf(stderr, "[REL2BIN] Table: %s\n", tbl_name ? tbl_name : "NULL");
//     if (!tbl_name) {
//         return sql_error(sql, 02, SQLSTATE(42000) "Table name cannot be NULL");
//     }
//     n = n->next;
    
//     int target_dim = n->data.i_val;
//     fprintf(stderr, "[REL2BIN] Target dimension: %d\n", target_dim);
//     if (target_dim <= 0) {
//         return sql_error(sql, 02, SQLSTATE(42000) "Target dimension must be positive");
//     }
//     n = n->next;
    
//     char *model_name = n->data.sval;
//     fprintf(stderr, "[REL2BIN] Model name: %s\n", model_name ? model_name : "NULL");
    
//     /* 2. 绑定源表并获取特征列 */
//     fprintf(stderr, "[REL2BIN] Binding table '%s'\n", tbl_name);
//     sql_table *src_table = mvc_bind_table(sql, cur_schema(sql), tbl_name);
//     if (!src_table) {
//         fprintf(stderr, "[REL2BIN] ERROR: Table '%s' not found\n", tbl_name);
//         return sql_error(sql, 02, SQLSTATE(42S02) "TABLE '%s' does not exist", tbl_name);
//     }
    
//     if (ol_length(src_table->columns) < 2) {
//         return sql_error(sql, 02, SQLSTATE(42000) "Table must have at least 2 columns");
//     }
    
//     stmt *tid = stmt_tid(be, src_table, 0);
//     node *col_node = src_table->columns->l->h;
//     col_node = col_node->next; 
//     sql_column *feature_col = col_node->data;
    
//     fprintf(stderr, "[REL2BIN] Feature column: %s (type ID: %d)\n", 
//             feature_col->base.name, feature_col->type.type->localtype);
    
//     stmt *col_stmt = stmt_col(be, feature_col, tid, tid->partition);
//     if (!col_stmt) return sql_error(sql, 02, SQLSTATE(42000) "Failed to get col stmt");

//     /* 3. 生成训练模型的 MAL 指令 */
//     fprintf(stderr, "[REL2BIN] Generating PCA instruction\n");
    
//     InstrPtr q = newStmt(be->mb, "batcalc", "pcatrain");
//     if (!q) return sql_error(sql, 10, SQLSTATE(HY013) MAL_MALLOC_FAIL);
...
//     // 获取结果变量号
//     int result_var = getArg(q, 0);
//     fprintf(stderr, "[REL2BIN] PCA result stored in X_%d\n", result_var);
    
//     stmt *model_stmt = stmt_direct_func(be, q);
//     if (!model_stmt) return sql_error(sql, 02, SQLSTATE(42000) "PCA stmt failed");

//     /* 4. 存储逻辑 (Append 结果到目标表) */
//     sql_table *dst_table = mvc_bind_table(sql, cur_schema(sql), model_name);
//     if (!dst_table) {
//         return sql_error(sql, 02, SQLSTATE(42S02) "Model table missing");
//     }

//     sql_column *dst_col = ol_first_node(dst_table->columns)->data;
//     fprintf(stderr, "[REL2BIN] Appending to %s.%s\n", model_name, dst_col->base.name);

//     stmt *target_bat = stmt_bat(be, dst_col, 0, 0); 
//     stmt *ins_stmt = stmt_append(be, target_bat, model_stmt);
    
//     if (!ins_stmt) return sql_error(sql, 02, SQLSTATE(42000) "Append failed");

//     sql->type = Q_UPDATE; 
//     fprintf(stderr, "[REL2BIN] PCA append statement created successfully\n");
//     fprintf(stderr, "========== REL2BIN PCATRAIN DEBUG END ==========\n");
...
//     dnode *n = info->h;
//     char *tbl_name = n->data.sval;
//     if (!tbl_name) return sql_error(sql, 02, SQLSTATE(42000) "Table name cannot be NULL");
    
//     n = n->next;
//     int target_dim = n->data.i_val;
//     if (target_dim <= 0) return sql_error(sql, 02, SQLSTATE(42000) "Target dimension must be positive");
    
//     n = n->next;
//     char *model_name = n->data.sval;
    
//     list *stmts = sa_list(sql->sa);
    
//     /* 2. 检查目标表是否存在，并强制要求只有 1 列 */
//     fprintf(stderr, "[REL2BIN] Checking target model table '%s'...\n", model_name);
//     sql_schema *schema = cur_schema(sql);
//     sql_table *dst_table = mvc_bind_table(sql, schema, model_name);
    
//     if (!dst_table) {
//         return sql_error(sql, 02, SQLSTATE(42S02) 
//                          "Target model table '%s' does not exist. Please create it first.", 
//                          model_name);
//     }
    
//     // 强制限制：表必须且只能有 1 列
//    if (!dst_table->columns || ol_length(dst_table->columns) != 1) {
//          return sql_error(sql, 02, SQLSTATE(42000) 
//                          "Target model table '%s' must have exactly ONE column. "
//                          "Please recreate it as: CREATE TABLE %s (model_data CLOB);", 
//                          model_name, model_name);
//     }

//     /* 3. 绑定源表并获取特征列 */
//     sql_table *src_table = mvc_bind_table(sql, schema, tbl_name);
//     if (!src_table) {
//         return sql_error(sql, 02, SQLSTATE(42S02) "Source TABLE '%s' does not exist", tbl_name);
//     }
    
//     if (ol_length(src_table->columns) < 2) {
//         return sql_error(sql, 02, SQLSTATE(42000) "Source table must have at least 2 columns");
//     }
...
//     /* 4. 生成训练模型的 MAL 指令 (先生成，确保 X_18 的编号和位置靠前) */
//     fprintf(stderr, "[REL2BIN] Generating batcalc.pcatrain instruction\n");
    
//     // A. 先准备参数 (这些不依赖 X_18)
//     stmt *dim_stmt = stmt_atom_int(be, target_dim);
//     // B. 生成 PCA 指令 (产出 X_18)
//     InstrPtr q = newStmt(be->mb, "batcalc", "pcatrain");
//     if (!q) return sql_error(sql, 10, SQLSTATE(HY013) MAL_MALLOC_FAIL);
    
//     q = pushArgument(be->mb, q, col_stmt->nr);
//     q = pushArgument(be->mb, q, dim_stmt->nr);
//     setVarType(be->mb, getArg(q, 0), newBatType(TYPE_str));
    
//     stmt *model_stmt = stmt_direct_func(be, q);
//     if (!model_stmt) return sql_error(sql, 02, SQLSTATE(42000) "PCA stmt failed");

//     /* 5. 存储逻辑 - 严格排在 pcatrain 之后 */
//     sql_column *dst_col = ol_first_node(dst_table->columns)->data;
//     fprintf(stderr, "[REL2BIN] Generating OVERWRITE instructions for %s.%s\n", model_name, dst_col->base.name);

//     // 5.1 获取目标表信息 (现在调用它们，生成的指令物理位置就在 pcatrain 之后)
//     stmt *dst_tids = stmt_tid(be, dst_table, 0);
//     if (!dst_tids) return sql_error(sql, 02, SQLSTATE(42000) "Failed to get TIDs for overwrite");

//     // 5.2 官方删除指令 (会消费 mvc)
//     stmt *del_stmt = stmt_delete(be, dst_table, dst_tids);
//     if (!del_stmt) return sql_error(sql, 02, SQLSTATE(42000) "Failed to generate delete stmt");

//     // 5.3 包装目标列
//     stmt *dst_col_stmt = stmt_col(be, dst_col, dst_tids, dst_tids->partition);
    
//     // 5.4 官方追加指令 (现在消费 X_18，安全通过 chkFlow 检查)
//     stmt *append_stmt = stmt_append(be, dst_col_stmt, model_stmt);
//     if (!append_stmt) return sql_error(sql, 02, SQLSTATE(42000) "Append failed");

//     // 将所有生成的语句按顺序加入列表，MonetDB 框架会按此列表导出结果
//     list_append(stmts, dim_stmt);
//     list_append(stmts, model_stmt);
//     list_append(stmts, dst_tids);
//     list_append(stmts, del_stmt);
//     list_append(stmts, append_stmt);

//     sql->type = Q_UPDATE; 
//     return stmt_list(be, stmts);
// }
...
//     /* 1. 纯数据解析阶段 */
//     dlist *info = rel->l;
//     char *tbl_name = info->h->data.sval;
//     int target_dim = info->h->next->data.i_val;
//     char *model_name = info->h->next->next->data.sval;

//     /* 2. 强制性语义检查 */
//     sql_table *dst_table = mvc_bind_table(sql, schema, model_name);
//     if (!dst_table) 
//         return sql_error(sql, 02, SQLSTATE(42S02) "Target model table '%s' does not exist.", model_name);
    
//     if (!dst_table->columns || ol_length(dst_table->columns) != 1) {
//         return sql_error(sql, 02, SQLSTATE(42000) "Target table '%s' must have exactly ONE column.", model_name);
//     }

//     sql_table *src_table = mvc_bind_table(sql, schema, tbl_name);
//     if (!src_table || ol_length(src_table->columns) < 2) {
//         return sql_error(sql, 02, SQLSTATE(42S02) "Source table '%s' error or too few columns.", tbl_name);
//     }

//     /* 3. 生产阶段：必须在这里彻底完成 X_18 的“物理落位” */
    
//     // 3.1 准备原料
//     stmt *src_tids = stmt_tid(be, src_table, 0); 
//     sql_column *feature_col = ol_first_node(src_table->columns)->next->data;
//     stmt *col_stmt = stmt_col(be, feature_col, src_tids, src_tids->partition);
//     stmt *dim_stmt = stmt_atom_int(be, target_dim);
    
//     // 3.2 生成并压入 pcatrain 指令
//     InstrPtr q_pca = newStmt(be->mb, "batcalc", "pcatrain");
//     if (!q_pca) return sql_error(sql, 10, SQLSTATE(HY013) MAL_MALLOC_FAIL);
    
//     q_pca = pushArgument(be->mb, q_pca, col_stmt->nr);
//     q_pca = pushArgument(be->mb, q_pca, dim_stmt->nr);
// 		q_pca->unsafeProp = 1;
    
//     // 强制声明返回值类型为字符串 BAT
//     setVarType(be->mb, getArg(q_pca, 0), newBatType(TYPE_str));

//     // 【关键】：手动压入 mb，这步操作给 X_18 发了“准生证”，固定了其在扫描顺序中的位置
//     pushInstruction(be->mb, q_pca); 

//     // 使用官方 API 包装成 stmt，替代会报错的 stmt_create
//     stmt *model_stmt = stmt_direct_func(be, q_pca);
//     if (!model_stmt) return sql_error(sql, 02, SQLSTATE(42000) "PCA stmt failed");

//     /* 4. 消费阶段：现在 X_18 的初始化位置已经排在前面，append 引用它不再报错 */
    
//     // 修复 dst_col 未定义的问题：从目标表中获取第一列
//     sql_column *dst_col = ol_first_node(dst_table->columns)->data;

//     // 获取目标表 TIDs (产生新指令，PC 会排在 q_pca 之后)
//     stmt *dst_tids = stmt_tid(be, dst_table, 0); 
    
//     // 生成删除指令
//     stmt *del_stmt = stmt_delete(be, dst_table, dst_tids);
    
//     // 生成追加指令 (引用 model_stmt)
//     stmt *dst_col_stmt = stmt_col(be, dst_col, dst_tids, dst_tids->partition);
//     stmt *append_stmt = stmt_append(be, dst_col_stmt, model_stmt);

// 		// 【关键新增 2】：强制让 append 产生的变量被消费
//     // 我们手动生成一个 io.print，或者利用 sql.affectedRows 引用它
//     InstrPtr q_aff = newStmt(be->mb, "sql", "affectedRows");
//     q_aff = pushArgument(be->mb, q_aff, be->mvc_var); // 传入当前 mvc
//     q_aff = pushInt(be->mb, q_aff, 1);               // 假装影响了 1 行
//     pushInstruction(be->mb, q_aff);

//     /* 5. 组装结果 */
//     list_append(stmts, src_tids);
//     list_append(stmts, col_stmt);
//     list_append(stmts, dim_stmt);
//     list_append(stmts, model_stmt); 
//     list_append(stmts, dst_tids);
//     list_append(stmts, del_stmt);
...
//     sql->type = Q_UPDATE; 
//     fprintf(stderr, "[REL2BIN] PCA flow generated successfully\n");
//     return stmt_list(be, stmts);
// }
...
{   
    ...
    fprintf(stderr, "\n[DEBUG-PHYSICAL] 1. rel2bin_pcatrain started. rel=%p, rel->op=%d\n", (void*)rel, rel->op);

    /* ==========================================================
     * 1. 极其安全的结构解包 (防优化器“魔改”版)
     * ========================================================== */
    sql_rel *src_rel = rel->l;
    sql_rel *dst_rel = rel->r;

    fprintf(stderr, "[DEBUG-PHYSICAL] 2. Initial unpack: src_rel=%p, dst_rel=%p\n", (void*)src_rel, (void*)dst_rel);

    if (!src_rel) return sql_error(sql, 02, SQLSTATE(42000) "PCA Error: src_rel is NULL");
    if (!dst_rel) return sql_error(sql, 02, SQLSTATE(42000) "PCA Error: dst_rel is NULL");

    // 剥离优化器外壳
    int loop_cnt = 0;
    while (src_rel && (src_rel->op == op_project || src_rel->op == op_table || src_rel->op == op_basetable)) {
        fprintf(stderr, "[DEBUG-PHYSICAL] 3a. src_rel peeling layer %d. op=%d\n", loop_cnt++, src_rel->op);
        if (src_rel->op == op_basetable) break; // 找到了真正的表节点
        src_rel = src_rel->l;
    }

    loop_cnt = 0;
    while (dst_rel && (dst_rel->op == op_project || dst_rel->op == op_table || dst_rel->op == op_basetable)) {
        fprintf(stderr, "[DEBUG-PHYSICAL] 3b. dst_rel peeling layer %d. op=%d\n", loop_cnt++, dst_rel->op);
        if (dst_rel->op == op_basetable) break; // 找到了真正的表节点
        dst_rel = dst_rel->l;
    }

    fprintf(stderr, "[DEBUG-PHYSICAL] 4. Peeling done. Final src_rel=%p, dst_rel=%p\n", (void*)src_rel, (void*)dst_rel);

    // 此时 src_rel 和 dst_rel 应该是 op_basetable，它们的 l 指针才是 sql_table
    sql_table *src_table = src_rel ? src_rel->l : NULL;
    sql_table *dst_table = dst_rel ? dst_rel->l : NULL;
    int target_dim = rel->flag;

    fprintf(stderr, "[DEBUG-PHYSICAL] 5. Tables extracted. src_table=%p, dst_table=%p, dim=%d\n", 
            (void*)src_table, (void*)dst_table, target_dim);

    /* ==========================================================
     * 2. 极致防御：限制检查
     * ========================================================== */
    if (!src_table || !dst_table) {
         return sql_error(sql, 02, SQLSTATE(42S02) "PCA Error: Tables lost during optimization.");
    }
    
    // 【高危预警】：如果 src_table/dst_table 强转失败，这里就会崩溃
    fprintf(stderr, "[DEBUG-PHYSICAL] 6. Verifying table pointers. src_name='%s', dst_name='%s'\n", 
            src_table->base.name, dst_table->base.name);

    if (ol_length(dst_table->columns) != 1) {
         return sql_error(sql, 02, SQLSTATE(42000) "PCA Error: Target table must have 1 column.");
    }

    /* ==========================================================
     * 3. 核心计算：生成 PCA 训练指令
     * ========================================================== */
    fprintf(stderr, "[DEBUG-PHYSICAL] 7. Building source statements...\n");
    stmt *src_tids = stmt_tid(be, src_table, 0); 
    sql_column *feature_col = ol_first_node(src_table->columns)->data;
    stmt *col_stmt = stmt_col(be, feature_col, src_tids, src_tids->partition);
    stmt *dim_stmt = stmt_atom_int(be, target_dim);

    fprintf(stderr, "[DEBUG-PHYSICAL] 8. Binding pcatrain func...\n");
    sql_subfunc *f = sql_find_func(sql, "sys", "pcatrain", 2, F_FUNC, false, NULL);
    if (!f) return sql_error(sql, 02, SQLSTATE(42000) "PCA Error: 'pcatrain' function not found.");

    list *ops = sa_list(sql->sa);
    list_append(ops, col_stmt);
    list_append(ops, dim_stmt);
    stmt *model_stmt = stmt_Nop(be, stmt_list(be, ops), NULL, f, NULL);

    /* ==========================================================
     * 4. 覆写逻辑
     * ========================================================== */
    fprintf(stderr, "[DEBUG-PHYSICAL] 9. Building Delete & Append statements...\n");
    stmt *dst_tids = stmt_tid(be, dst_table, 0); 
    stmt *del_stmt = stmt_delete(be, dst_table, dst_tids);
    list_append(l, del_stmt); 

    sql_column *dst_col = ol_first_node(dst_table->columns)->data;
    stmt *dst_col_stmt = stmt_col(be, dst_col, NULL, 0); 
    stmt *append_stmt = stmt_append(be, dst_col_stmt, model_stmt);
    list_append(l, append_stmt); 

    /* ==========================================================
     * 5. 官方 DML 统计闭环
     * ========================================================== */
    fprintf(stderr, "[DEBUG-PHYSICAL] 10. Building aggregations...\n");
    sql_subtype *bt = tail_type(append_stmt);
    sql_subfunc *cnt_func = sql_bind_func(sql, "sys", "count", bt, NULL, F_AGGR, true, true);
    cnt = stmt_aggr(be, append_stmt, NULL, NULL, cnt_func, 1, 0, 1);
    list_append(l, cnt);

    fprintf(stderr, "[DEBUG-PHYSICAL] 11. Adding to rowcount accumulator...\n");
    if (add_to_rowcount_accumulator(be, cnt->nr) < 0) {
        return sql_error(sql, 10, SQLSTATE(HY013) MAL_MALLOC_FAIL);
    }

    if (dst_table->s && isGlobal(dst_table) && !isGlobalTemp(dst_table)) {
        stmt_add_dependency_change(be, dst_table, cnt);
    }

    sql->type = Q_UPDATE; 
    fprintf(stderr, "[DEBUG-PHYSICAL] 12. rel2bin_pcatrain completed successfully!\n");
    return stmt_list(be, l);
}